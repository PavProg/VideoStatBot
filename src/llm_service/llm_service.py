...
class YandexMLGPTQueryService:
    ...
    def _create_sql_prompt(self, user_query: str) -> list:
        ...
        6. Для подсчета количества используй COUNT(*).
        7. Для суммирования используй SUM(поле).
        8. Для среднего значения используй AVG(поле).
        9. Для максимального/минимального используй MAX(поле)/MIN(поле).
        10. Всегда возвращай ТОЛЬКО SQL-запрос, без пояснений, без обратных кавычек ```, без markdown.
        11. Если не можешь создать запрос, верни 'NULL'.
        12. Для вопросов о датах (например "27 ноября 2025") используй функцию DATE(): WHERE DATE(created_at) = '2025-11-27'
        13. Для вопросов о "новых просмотрах" используй delta_views_count > 0
        """
        }
        user_message = {
            'role': 'user',
